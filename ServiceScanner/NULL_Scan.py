...
def NULL_Scan(host, port, probe):
    ...
    if Null_probe["totalwaitms"] is not None:
        client_socket.settimeout(0.1)  # 为增加速度，使用0.1，正式应该为int(Null_probe["totalwaitms"])//1000
    else:
    ...
    try:
        ...
        feedback = client_socket.recv(1024).decode('utf-8', 'ignore')  # ignore表示出现乱码后忽略
    except Exception as e:
        return 0
    finally:
        # 匹配部分
        if feedback is not None:
            for each in Null_probe["matches"]:
