...
from scan_mac import IP2MAC, MAC2Device
from vuldetect.vul_matcher import detect_vul
        ...
        def run(self):
            ...
            while True:
                ...
                try:
                    ...
                    if result_code == 0:
                        res_list = Service_Scan.Service_Scan(ip, port)
                        if len(res_list) > 0:
                            if len(res_list) == 4:
                                print(str(res_list[3]))
                                vul_list = detect_vul(res_list[1])
                                for i in range(len(vul_list)):
                                    print("Node: " + vul_list[i][0] + " Summary:" + vul_list[i][1])
                            else:
                                print(str(res_list[2]))
                                vul_list = detect_vul(res_list[1])
                                for i in range(len(vul_list)):
                                    print("Node: " + vul_list[i][0] + " Summary:" + vul_list[i][1])
                        else:
                            service_name = find_service_name(port)
                            print("[+]On port " + str(port) + "\nService: " + service_name)
                            vul_list = detect_vul(service_name)
                            for i in range(len(vul_list)):
                                print("Node: " + vul_list[i][0] + " Summary:" + vul_list[i][1])
...
/_______  /\___  \___  |   __/|____| \____/|____/ \___  |___|  /
    ...
    def help(self):
        help = """
[Usage]
    python deeptouch.py -h                                   帮助文档
    python deeptouch.py -i ip -p [port_scope|top_num]        限定端口范围，扫描ip的开放端口
    python deeptouch.py -i ip --mac                          扫描ip对应的硬件信息
    python deeptouch.py -u url -p [port_scope|top_num]       限定端口范围，扫描域名的开放端口
    python deeptouch.py -u url --draw                        画出从本机到域名的拓扑图
    python deeptouch.py -n network --os                      显示该网段下的操作系统
    python deeptouch.py -n network --draw                    画出该网段下的拓扑图
    python deeptouch.py -n network --mac                     显示该网段下的硬件信息
    python deeptouch.py -w url                               扫描web的框架、服务等信息
    \n\n[Example]
    python deeptouch.py -i 127.0.0.1 -p 1-1000 
    python deeptouch.py -i 192.168.1.1 --mac
    python deeptouch.py -u https://www.baidu.com -p 1-65535
    python deeptouch.py -u www.baidu.com --draw
    python deeptouch.py -n 192.168.1.0/24 --os
    python deeptouch.py -n 192.168.1.0/24 --draw
    python deeptouch.py -n 192.168.1.0/24 --mac
    python deeptouch.py -w https://www.douban.com
...
def main():
    ...
    if len(args) == 5:
        ...
        if args[3] == '--os':
            ...
            for i in range(1, 255):
                ip = prefix + str(i)
                t = threading.Thread(target=scan_host.ttl_scan, args=(ip, ))
                t.start()
                time.sleep(0.1)
    ...
    elif len(args) == 4 and args[1] == '-u' and args[3] == '--draw':
        # ['portscaner_v3.py', '-u', 'url', '--draw']
        dst_ip = port_scanner.get_ip_by_name(args[2])
        host_ip = draw_topology.get_host_ip()
        iplist_list = []
    ...
    elif len(args) == 4 and args[1] == '-i' and args[3] == '--mac':
        # ['portscaner_v3.py', '-i', 'ip', '--mac']
        dst_ip = args[2]
        print(dst_ip)
        msg_list = [['    ip       ', '     mac     ', '     info    ']]
        g = IP2MAC()
